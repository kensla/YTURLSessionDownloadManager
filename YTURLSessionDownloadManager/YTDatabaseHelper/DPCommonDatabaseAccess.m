...
@implementation DPCommonDatabaseAccess
- (instancetype)init {
    self = [super init];
    if (self) {
        self.databaseAccessTemplate = [[DPDatabaseAccessTemplate alloc] init];
    }
    return self;
}
...
- (BOOL)isTableExistWithTableName:(NSString *)tableName inDatabase:(FMDatabase *)database {
    ...
    NSArray *array = [self.databaseAccessTemplate openDatabase:database
        withExecuteQueryBlock:^FMResultSet *{
...
#endif
    return [self.databaseAccessTemplate openDatabase:database
                                          actionDesc:actionDesc
                              withExecuteUpdateBlock:^BOOL {
